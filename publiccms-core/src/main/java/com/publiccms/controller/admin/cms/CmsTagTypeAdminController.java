...
public class CmsTagTypeAdminController extends AbstractController {
        ...
        if (null != entity.getId()) {
            ...
            if (null != entity) {
                logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
                        LogLoginService.CHANNEL_WEB_MANAGER, "update.tagType", RequestUtils.getIpAddress(request),
            ...
            service.save(entity);
            logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
                    LogLoginService.CHANNEL_WEB_MANAGER, "save.tagType", RequestUtils.getIpAddress(request),
                    CommonUtils.getDate(), JsonUtils.getString(entity)));
        }
    ...
    public String delete(Integer id, String _csrf, HttpServletRequest request, HttpSession session, ModelMap model) {
        ...
        if (null != entity) {
            ...
            service.delete(id);
            logOperateService.save(new LogOperate(site.getId(), ControllerUtils.getAdminFromSession(session).getId(),
                    LogLoginService.CHANNEL_WEB_MANAGER, "delete.tagType", RequestUtils.getIpAddress(request),
